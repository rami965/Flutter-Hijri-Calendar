...
class _DatePickerHeader extends StatelessWidget {
  const _DatePickerHeader({Key? key, required this.hSelectedDate, required this.mode, required this.onModeChanged, required this.orientation})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
      ...
      child: _DateHeaderButton(
        ...
        child: Semantics(
          ...
          child: FittedBox(
            ...
            child: Text(hSelectedDate.toFormat("DD,dd MMMM"), style: dayStyle),
          ),
        ),
      ),
    );
    ...
    return Container(
      ...
      padding: padding,
      color: backgroundColor,
      child: Column(mainAxisAlignment: mainAxisAlignment, crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[yearButton, dayButton]),
    );
  }
}

class _DateHeaderButton extends StatelessWidget {
  const _DateHeaderButton({Key? key, this.onTap, this.color, required this.child}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: InkWell(
        ...
        onTap: onTap,
        child: Container(padding: const EdgeInsets.symmetric(horizontal: 8.0), child: child),
      ),
    );
  }
}
...
class _HijriMonthPickerState extends State<HijriMonthPicker> {
  ...
  void didUpdateWidget(HijriMonthPicker oldWidget) {
    ...
    if (!widget.selectedDate.isAtSameMomentAs(oldWidget.selectedDate.hYear, oldWidget.selectedDate.hMonth, oldWidget.selectedDate.hDay)) {
      final int monthPage = _monthDelta(widget.firstDate, widget.selectedDate);
      _dayPickerController = PageController(initialPage: monthPage);
      _handleMonthPageChanged(monthPage);
    }
  ...
  void _updateCurrentDate() {
    ...
    Duration timeUntilTomorrow = tomorrow
        ...
        .difference(_todayDate.hijriToGregorian(_todayDate.hYear, _todayDate.hMonth, _todayDate.hDay)); //tomorrow.difference(_todayDate);
  ...
  HijriCalendar _addMonthsToMonthDate(HijriCalendar monthDate, int monthsToAdd) {
    var x = HijriCalendar.addMonth(monthDate.hYear + (monthDate.hMonth + monthsToAdd) ~/ 12, monthDate.hMonth + monthsToAdd % 12);
    return x;
  }
          ...
          PositionedDirectional(
            ...
            child: Semantics(
              ...
              child: IconButton(
                ...
                tooltip: _isDisplayingFirstMonth ? null : '${localizations.previousMonthTooltip} ${_previousMonthDate.toString()}',
          ...
          PositionedDirectional(
            ...
            child: Semantics(
              ...
              child: IconButton(
                ...
                tooltip: _isDisplayingLastMonth ? null : '${localizations.nextMonthTooltip} ${_nextMonthDate.toString()}',
    ...
    for (int i = 0; true; i += 1) {
      ...
      if (day < 1) {
        ...
        final HijriCalendar dayToBuild = HijriCalendar()
          ...
          ..hDay = day;
        final bool disabled =
            dayToBuild.isAfter(lastDate.hYear, lastDate.hMonth, lastDate.hDay) ||
        ...
        final bool isSelectedDay = selectedDate.hYear == year && selectedDate.hMonth == month && selectedDate.hDay == day;
        if (isSelectedDay) {
          // The selected day gets a circle background highlight, and a contrasting text color.
          itemStyle = themeData.textTheme.bodyLarge?.copyWith(color: themeData.colorScheme.onSecondary);
          ...
          SizedBox(
            ...
            child: Center(
              child: ExcludeSemantics(
                child: Text("${displayedMonth.toFormat("MMMM")} ${displayedMonth.hYear}", style: themeData.textTheme.titleMedium),
              ),
            ),
          ),
          Flexible(
...
class HijriYearPicker extends StatefulWidget {
  ...
  /// Rarely used directly. Instead, typically used as part of the dialog shown
  /// by [showDatePicker].
  HijriYearPicker({Key? key, required this.selectedDate, required this.onChanged, required this.firstDate, required this.lastDate})
    : assert(!firstDate.isAfter(lastDate.hYear, lastDate.hMonth, lastDate.hDay)),
      super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        final TextStyle? itemStyle = isSelected ? themeData.textTheme.headlineSmall?.copyWith(color: themeData.colorScheme.secondary) : style;
        return InkWell(
